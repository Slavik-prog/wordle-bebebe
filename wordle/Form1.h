...
namespace Wordle {
    ...
    private:
        ...
        // 14. Подписка на события изменения текста
        void SubscribeToTextChangedEvents()
        {
            for (int i = 0; i < 30; i++)
            {
                textBoxes[i]->TextChanged += gcnew System::EventHandler(this, &Form1::TextBox_TextChanged); // Подписка на событие изменения текста
            }
        }

        // 15. Подписка на события нажатия клавиш
        ...
        // 16. Обработка нажатия клавиш в текстовом поле
        ...
        // 18. Выделение неправильных букв
        ...
        // 19. Обработка изменения текста в текстовом поле
        ...
        // 20. Проверка, заполнено ли слово
        bool IsCompleteWord()
        {
            for (int i = attemptCount * 5; i < (attemptCount + 1) * 5; i++)
            {
        ...
        void ShowMenu()
        {
            gamePanel->Visible = false; // Скрыть игровую панель
            menuPanel->Visible = true; // Показать панель меню
        }



        // 22. Очистка игрового состояния
        void ClearGame() {
            for (int i = 0; i < 30; i++) {
                textBoxes[i]->Text = "";
                textBoxes[i]->BackColor = System::Drawing::Color::White;
                textBoxes[i]->ReadOnly = false; // Разблокировка всех текстовых полей
            }
            attemptCount = 0;
            SelectRandomWord();
            textBoxes[0]->Focus();
    ...
    private:
        // 23. Дополнительные настройки при загрузке формы
